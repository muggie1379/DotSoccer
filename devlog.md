...
- Unity 에디터 창만 정확히 오려서 스크린샷으로 저장하는 방법을 확인했다. 별도
  플러그인 설치 없이 Windows 기본 PowerShell(`System.Drawing` +
  `user32.dll`의 `PrintWindow`)만으로, 실행 중인 Unity 프로세스의 창 핸들을
  찾아 그 창만 캡처할 수 있었다 (전체 화면 캡처와 달리 다른 창이 같이 찍히지
  않는다). 재사용할 수 있게 `tools/capture-unity-window.ps1` 스크립트로
  만들어뒀다.
- 위 캡처 스크립트로 실제 Play 모드를 찍어보다가, 자책골이 끝없이 반복되는
  버그를 발견했다. 원인은 `GameConfig.Player1Spawn`/`Player2Spawn`이 정확히
  자기 골대 열(x=0, x=18)에 있었던 것 -- 골대 판정 범위(`GoalZoneSize`)의 y
  범위에도 걸쳐 있어서, 스폰하자마자 공을 든 채로 이미 자기 골대 안에 있는
  셈이 됐다. 게다가 자책골이 나면 실점한 팀(=자책골을 넣은 선수 자신)이 다시
  공을 받아 같은 자리에서 시작하는 구조라, 한 번 발동하면 끝없이 반복됐다.
  스폰 위치를 골대 열에서 안쪽으로 옮겨서(x=3, x=15) 해결했다.
  - Unity 씬에 이미 저장돼 있는 컴포넌트 값은 C# 코드의 기본값을 바꿔도
    갱신되지 않는다는 것도 이번에 다시 확인했다 (직렬화된 값이 코드 기본값을
    덮어씀). `MatchController`의 `config` 필드도 mcp-unity의
    `update_component`로 씬에 저장된 값을 직접 갱신해야 했다.
  - 값이 실제로 반영됐는지는 스크린샷의 픽셀 위치만으로 판단하기 애매해서,
    `MatchController.Start()`에 임시로 `Debug.Log`를 추가해 런타임 좌표를
    콘솔에서 직접 확인하는 방식으로 검증했다 (확인 후 제거).
...
- DevLog 번호를 재배치했다: 기존에는 #0이 "기획 및 로드맵", #1이 "환경설정"
  이었는데, 사용자 지시로 #0을 "환경설정", #1을 "DotSoccer 기획 및 로드맵"
  으로 바꿨다. "환경설정" 글은 제목만 고쳐서 그대로 #0으로 옮겼고, "기획 및
  로드맵" 글은 번호가 겹치는 문제가 있어 같은 내용으로 새 게시글을 #1로
  발행한 뒤 기존 글을 삭제했다. SKILL.md의 DevLog 목차도 이 순서로
  갱신했다.
...
- DotSoccer 클라이언트: 기존 로컬 프로토타입을 서버 연동을 염두에 둔 구조로
  ...
  Play 모드 동작까지 확인했다.
- Unity 에디터 창 스크린샷 캡처 방법을 확립하고, 그걸로 Play 모드를 실제
  찍어보다가 자책골 무한 반복 버그(스폰 위치가 자기 골대 열 위에 있던 문제)를
  발견해서 고쳤다.
...
- DevLog 번호를 재배치했다 (환경설정=#0, DotSoccer 기획 및 로드맵=#1).
...
- WSL 미설치로 서버 빌드/실행 검증이 계속 보류 중이다. 다음 단계로 WSL(또는
  ...
  한다.
- DevLog #1의 "반도체 자동화 경험과의 연결" 부분을 더 강화해서 쓰기 <!-- 사용자 요청, 오늘은 작업 안 함. 블로그(History 등)에는 반영하지 않고 devlog.md에만 메모 -->.
...
<!-- published 2026-09-13 to velog DevLog #0 (환경설정; 발행 당시엔 #1이었으나 이후 #0으로 재배치됨) -->
